...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        public async Task<Movie> GetMovieAsync(string movieId, CancellationToken cancellationToken = default)
        {
            try
            {
                var res = await _moviesCollection.Aggregate()
                    .Match(Builders<Movie>.Filter.Eq(x => x.Id, movieId))
                    .Lookup<Movie, Comment, Movie>(_commentsCollection, m => m.Id, c => c.MovieId, m => m.Comments)
                    // Ticket: Get Comments
            ...
            catch (Exception ex)
            {
                ...
                // throw.
                if (ex is System.FormatException) return null;
                throw;
            }
        }
            ...
            )
        {
            ...
            //   .Project(...)
            //   .ToListAsync(cancellationToken);
            var sortFilter = Builders<Movie>.Sort.Descending(m => m.Title);
            var projectionFilter = Builders<Movie>.Projection
                .Include(m => m.Title)
                .Include(m => m.Id);
                
            Func<Movie, bool> SearchLogic = m =>
            {
                if (m.Countries == null) return false;
                foreach(var i in countries)
                {
                    foreach (var j in m.Countries)
                    {
                        if (i == j) return true;
                    }
                }
            ...
            var searchFilter = Builders<Movie>.Filter.Where(m => SearchLogic(m));
            var movies = await _moviesCollection
                .Find(Builders<Movie>.Filter.Empty)
                .Sort(sortFilter)
                .ToListAsync(cancellationToken);
            List<MovieByCountryProjection> result = new();
            foreach(var i in movies)
            {
                if (SearchLogic(i))
                {
                    result.Add(new MovieByCountryProjection()
                    {
                        Title = i.Title,
