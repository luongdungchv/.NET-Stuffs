    ...
    public class UsersRepository
    {
        ...
        */
        ...
        public async Task<User> GetUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            var result = await _usersCollection.Find(Builders<User>.Filter.Where(u => u.Email == email)).FirstOrDefaultAsync();
            return result;
        }
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var filter = Builders<Session>.Filter.Where(s => s.UserId == user.Email);
                var session = await _sessionsCollection.Find(
                    filter)
                    .FirstOrDefaultAsync();
                var s = "";
                if(session != null)
                {
                    ...
                    var updateBuilder = Builders<Session>.Update.Set(s => s.Jwt, user.AuthToken);
                    await _sessionsCollection.UpdateOneAsync(filter, updateBuilder);
                }
                else
                {
            ...
            Dictionary<string, string> preferences, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                */
                ...
                updateResult = await _usersCollection
